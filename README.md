...
The second stage of the pipeline uses a Modified Multi-Attention U-Net (MAU-Net) as the image recoloring network. The model takes a 4-channel tensor as input, consisting of the original RGB image concatenated with the semantic priority map generated by the frozen Stage 1 U-Net.

The priority map acts as a spatial guide that tells the MAU-Net which regions should receive stronger recoloring. High-priority regions, such as traffic lights and warning signs, are enhanced more noticeably so that they become easier to distinguish for red-green Color Vision Deficiency (CVD) users. At the same time, low-priority regions such as roads, sky, vegetation, buildings, and other background areas are preserved as much as possible to maintain a natural-looking image.
